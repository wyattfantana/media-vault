...
import { TikTokCardSkeleton } from '../components/VideoCardSkeleton';
...
export function TikTok() {
  ...
  useEffect(() => {
    const savedState = sessionStorage.getItem('tiktok-page-state');
    if (savedState) {
      try {
        ...
        console.error('Failed to restore TikTok state:', err);
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (videos.length > 0) {
      ...
      sessionStorage.setItem('tiktok-page-state', JSON.stringify(state));
    }
  ...
  const handleBrowse = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/search/extract?url=${encodeURIComponent(inputUrl)}&limit=50`,
        { credentials: 'include' }
      );
      ...
      if (res.ok) {
        ...
        alert('Failed to load TikTok content');
      }
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-100 mb-2">TikTok Browse</h1>
        <p className="text-gray-400">Browse TikTok user profiles and videos</p>
      </div>
            ...
            {category === 'custom' && (
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="e.g., TikTok Clips, Trends, etc."
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                  required
                />
              </div>
